<template>
	<view class="information">
		<view class="inforImg">
			<image style="width: 750upx;height: 700upx;" :src="info.photo_url" mode="aspectFill"></image>
		</view>
		<view class="IFcontent">
			<view style="margin-left: 23upx;">
				<view class="IFcontentTop">
					<view class="IFname">{{info.nickname}}</view>
					<image class="IFimg" :src="info.sex==2 ? '../../static/one/gender.png' : '../../static/one/gender1.png'"></image>
					<view class="IFgrade">{{info.levels}}级</view>
				</view>
				<view class="IFcontentB">
					<view>魅力值：{{info.zan}}</view>
					<view>身价：{{info.gold}}</view>
					<view class="IFaddress">{{info.location}}</view>
				</view>
			</view>
					...
					<swiper :current="tabIndex" @change="changeTab" :style="{height:swiperHeight+'px'}">
						<swiper-item>		
							...
							<view style="margin-left: 23upx;">
								<view>
									...
									<view class="photo">
										<image v-for="item in ph" lazy-load style="width: 120upx;margin:0 16upx;height: 120upx;border-radius: 10upx;" :src="'http://social.wgzy69.com/'+item.circle_url" mode="aspectFill"></image>
									</view>
								</view>
								<view class="IFalbum">
									<view class="IFalbumTop">
						...
						<swiper-item>
							<view class="swiper-item list1">
								<view class="moments__post" v-for="(post,index) in info.circle" :key="index" :id="'post-'+index">
									<view class="post-left">
										<image class="post_header" :src="info.photo_url"></image>
										<view class="name">
											<view class="post-username">{{info.nickname}}</view>
									...
									<view class="post_right post_right1">
										...
										<view class="thumbnails">
											<view :class="post.url.length === 1?'my-gallery':'thumbnail'" v-for="(image, index_images) in post.url" :key="index_images" @click="previewImage(post.url,index_images)">
												<image class="gallery_img" lazy-load mode="aspectFill" :src="'http://social.wgzy69.com/'+image.circle_url" :data-src="'http://social.wgzy69.com/'+image.circle_url" ></image>
											</view>
										</view>
										<view class="toolbar1">
											<view class="tool" @click="like(post)">
												<image :src="post.type=='1'?'../../static/one/love1.png':'../../static/one/redlove1.png'"></image>
		...
		methods: {
			...
			//朋友圈信息接口
				...
				previewImage(imageList, image_index) {
					// console.log("点击图片显示",imageList);
					// console.log("点击图片显示",image_index);
					var imgList=[];
