<template>
	<view>
		<view class='calendar-Time-header under_line'>
			<view class='calendar-lastMonth' @tap="subMonth" v-bind:class="{'calendar-btn-disabled' : lastDisabled }">
				<image style="width: 8px;height: 12px;" src="../../static/one/dateLeft.png" mode=""></image>
			</view>
			<text class='calendar-TimeH'>{{title_time}}</text>
			<view class='calendar-nextMonth' @tap="addMonth" v-bind:class="{'calendar-btn-disabled' : nextDisabled }">
				<image style="width: 8px;height: 12px;" src="../../static/one/dateRight.png" mode=""></image>
			</view>
			...
			<view class="calendar-row calendar-day" v-bind:style="{'transform':transformObj, 'transition-duration':transformTimeObj}" @touchstart="touchstart" @touchmove="touchmove" @touchend="touchend">
				...
				<view class="calendar-col aligncanter currentDays" 
				 v-for='item in dateList' v-bind:key='item.key' @tap="selectedDateFun(item.dateIndex)">
					<view class='dayValue'>
						<text class='calendar-date calendar-day-6-7' v-bind:class="{'calendar-active' : item.dateIndex == currentSelectTime
				, 'calendar-today' : item.isToday 
				, 'calendar-range' : item.isRanges
				, 'calendar-out-of-Date' : item.outOfDate }" v-if="item.isDaySunOrSat">{{item.dateIndex}}</text>
						<text v-else class='calendar-date' v-bind:class="{'calendar-active' : item.dateIndex == currentSelectTime
				, 'calendar-today' : item.isToday 
				, 'calendar-range' : item.isRanges
				, 'calendar-out-of-Date' : item.outOfDate }">{{item.dateIndex}}</text>
